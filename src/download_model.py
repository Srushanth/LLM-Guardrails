"""A module that downloads a model from HuggingFace Hub and prints its size."""
...
parser = argparse.ArgumentParser(
    description="A simple script that takes two input arguments and prints them"
)
...
if __name__ == "__main__":
    ...
    response = requests.head(
        f"https://huggingface.co/{args.repo_id}/resolve/{args.revision}/", timeout=10
    )
    ...
    for _ in tqdm(
        snapshot_download(
            ...
            local_dir=f"../models/{args.repo_id}/{args.revision}",
        ),
        total=file_count,
    ):
        pass
